...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const updateViolationSummary = () => {
        ...
        sortedPairKeys.forEach(key => {
            ...
            // Render row if the analysis object exists, regardless of the required distance value.
    ...
    const addSummaryRow = (data, factor, isFeasiblePair) => {
        ...
        const row = `<tr class="${rowClass}">
            <td data-label="Building Pair">${data.pairLabel}</td>
            <td data-label="Required Distance">${(data.reqDist * factor).toFixed(2)}</td>
            <td data-label="Available Distance">${(data.availDist * factor).toFixed(2)}</td>
            <td data-label="Shortfall (%)" class="${data.isViolation ? 'shortfall-cell' : ''}">${data.isViolation ? data.shortfall.toFixed(1) + '%' : 'N/A'}</td>
            <td data-label="Status" class="remarks-output ${data.isViolation ? 'violation' : 'safe'}">${data.isViolation ? '❌ Violation' : '✔️ Safe'}</td>
    ...
    const addSurroundingRow = () => {
        ...
        newRow.innerHTML = `
            <td class="proposed-bldg-name" data-label="Proposed Bldg (X)"></td>
            <td data-label="Surrounding Bldg (Y) Details"><input type="text" class="surrounding-bldg-name" placeholder="Name/No. of Surrounding Bldg"><div class="inline-group coord-group"><input type="text" class="surrounding-bldg-lat" placeholder="Latitude"><input type="text" class="surrounding-bldg-lon" placeholder="Longitude"></div><button type="button" class="map-btn">Set on Map</button></td>
            <td data-label="Orientation of X (PES)"><select class="pes-orientation">${orientationOptions}</select></td>
            <td data-label="Orientation of Y (ES)"><select class="es-orientation">${orientationOptions}</select></td>
            <td class="siqd-required" data-label="SIQD Required"></td>
            <td class="piqd-required" data-label="PIQD Required"></td>
            <td class="distance-available" data-label="Distance Available"></td>
            <td class="remarks-output" data-label="Remarks"></td>`;
        syncAndAnalyzeReverseTable();
    };
    ...
    const createReverseRowHTML = () => {
        ...
        return `
            <td class="sur-name-reverse" data-label="Surrounding Bldg (Y)"></td>
            <td data-label="NEC of Y (kg)"><input type="text" class="reverse-nec" placeholder="e.g., 1000"></td>
            <td data-label="Type of Y"><select class="reverse-type">${typeOptions}</select></td>
            <td data-label="Use Type"><select class="reverse-use"><option value="">Select Use</option><option value="Storage">Storage</option><option value="Process">Process</option></select></td>
            <td data-label="Orientation of Y (PES)"><select class="pes-orientation">${orientationOptions}</select></td>
            <td data-label="Orientation of X (ES)"><select class="es-orientation">${orientationOptions}</select></td>
            <td class="reverse-siqd" data-label="SIQD Required by Y"></td>
            <td class="reverse-piqd" data-label="PIQD Required by Y"></td>
            <td class="reverse-distance" data-label="Distance Available"></td>
            <td class="remarks-output" data-label="Remarks"></td>`;
    };
